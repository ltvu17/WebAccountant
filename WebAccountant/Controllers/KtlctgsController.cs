...
namespace WebAccountant.Controllers
{
    ...
    public class KtlctgsController : Controller
    {
        ...
        public KtlctgsController(TIENHIEU2024Context context) {
            _context = context;
        }

        [HttpGet]
        public async Task<IActionResult> Get(DataSourceLoadOptions loadOptions) {
            var ktlctgs = _context.Ktlctgs.Select(i => new {
                i.Lctg,
                i.Tenctg,
                i.Tkthue,
                i.CoTkdutkthue,
            ...
            // loadOptions.PrimaryKey = new[] { "Lctg" };
            // loadOptions.PaginateViaPrimaryKey = true;

            return Json(await DataSourceLoader.LoadAsync(ktlctgs, loadOptions));
        }
        ...
        public async Task<IActionResult> Post(string values) {
            ...
            var result = _context.Ktlctgs.Add(model);
        ...
        public async Task<IActionResult> Put(string key, string values) {
            var model = await _context.Ktlctgs.FirstOrDefaultAsync(item => item.Lctg == key);
            if(model == null)
