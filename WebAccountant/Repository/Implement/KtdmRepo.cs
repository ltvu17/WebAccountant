...
namespace WebAccountant.Repository.Implement
{
    public class KtdmRepo : IKtdmRepo
    {
        ...
        public async Task<string> ExportPDF(AddToKTSCDTO item)
        {
            var items = item.ktdmDTOs.ToList();
            var customer = (await _unitOfWork.KTDTPNDAO.GetAll()).FirstOrDefault(x=>x.Madtpn == item.Makh);
            ...
            if (customer != null && models.Count() > 0)
            {
                ...
                foreach (var model in models)
                {
                    ...
                    var insertPhieuHang = new Ktsc()
                    {
                        ...
                        Soct = item.SoCtu != null? item.SoCtu : "00000" + IdChungtu,
                ...
                var insertThuePhieuHang = new Ktsc()
                {
                    ...
                    NgayHd = item.NgayHToan,
                    Ngayct = item.NgayCtu,
                    Diengiai = "Thuế GTGT mua vào HĐ số : 00000" + IdChungtu + "," + item.NgayCtu.ToShortDateString(),
                    Tkno = "1331",
                    Madtpnco = customer.Madtpn,
                    Tkco = "331",
                    MaCt = "",
