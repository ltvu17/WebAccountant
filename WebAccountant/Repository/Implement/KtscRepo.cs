...
namespace WebAccountant.Repository.Implement
{
    public class KtscRepo : IKtscRepo
    {
        ...
        {
            var list = new List<PhieuMuaHangDTO>();
            var items = (await _unitOfWork.KTSCDAO.GetAll()).Where(s=>s.Lctg == "PNK" && !s.IdNghiepvu.Contains("CHIETKHAU_HDBR")).OrderByDescending(s => s.Ngayct).GroupBy(i => new
            {
                i.SoHd,
                i.Tenkh,
                i.Ngayct
            ...
            foreach (var item in items)
            {
                var groupItem = item.Where(s => s.Lctg == "PNK" && !s.Diengiai.Contains("Thuế GTGT mua vào HĐ") && !s.IdNghiepvu.Contains("CHIETKHAU") && !s.IdNghiepvu.Contains("VAT_PNK_PC") && !s.IdNghiepvu.Contains("VAT_RA") 
                                            && !s.Diengiai.Contains("Chiết khấu theo chứng từ")).ToList();
                var insertItem = new PhieuMuaHangDTO();
                double tongtien = 0;
                double thanhTien = 0;
        ...
        public async Task<bool> RefundPackageBuy(IEnumerable<double> sttSc)
        {
            var ktdms = new List<KtdmDTO>();
            var hinhthuctt = "Tiền Mặt";
            var makh = "";
            ...
            if (ktdms.Count() > 0)
            {
                ...
                await SavePhieuTraHangMuaToDB(formBanHangDTO);
            }
            return true;
        }
        public async Task<bool> SavePhieuTraHangMuaToDB(FormBanHangDTO item)
        {
            var items = item.ktdmDTOs.ToList();
            var newGuid = Guid.NewGuid();
            var customer = (await _unitOfWork.KTDTPNDAO.Find(s => s.Madtpn == item.Makh, 1, 1)).FirstOrDefault();
